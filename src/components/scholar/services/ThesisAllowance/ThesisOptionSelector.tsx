...
export function ThesisOptionSelector({ 
  ...
  return (
    <Card 
      ...
      className={cn(
        "relative transition-all duration-200 border-2 group",
        disabled
          ? "bg-gray-50 border-gray-200 cursor-not-allowed opacity-80"
          : "cursor-pointer bg-white border-transparent hover:border-dost-title/30 hover:shadow-lg",
        className
      )}
    >
      {/* Locked Icon */}
          ...
          <div className="space-y-1">
            <h3
              className={cn(
                ...
                disabled
                  ? "text-gray-500"
                  : "text-gray-900 group-hover:text-dost-title"
              )}
            >
              {title}
            </h3>
