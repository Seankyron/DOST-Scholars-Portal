...
interface RequestFormsRowProps {
  request: RequestFormDetails;
  onUpdate: () => void;
}

export function RequestFormsRow({ request, onUpdate }: RequestFormsRowProps) {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const { scholarInfo, currentPlacement, requestType, submissionInfo } = request;
